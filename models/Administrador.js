...
class Administrador{
    ...
    async new(nomeAdministrador, MatriculaAdministrador, password){
        try {
            var hash = await bcrypt.hash(password, 12)
            await knex.insert({nomeAdministrador, MatriculaAdministrador, password: hash}).table("administrador")            
            return {status: true}
        } catch (error) {
            console.error(error)
            return {status: false}
